...
class LeaveController extends Controller
{
    ...
    {
        if ( ! Auth::user()->isHR()) {
            $leaves = EmployeeLeaves::with('user.employee')->where('tl_id', Auth::user()->id)->orWhere('manager_id',
                Auth::user()->id)->paginate(15);
        } else {
        ...
        try {
            ...
            if ($request->button == 'Search') {
                ...
                if ( ! empty($column) && ! empty($string) && empty($dateFrom) && empty($dateTo)) {
                    ...
                    $dateTo   = date_format(date_create($request->dateTo), 'Y-m-d');
                    $dateFrom = date_format(date_create($request->dateFrom), 'Y-m-d');
                    $leaves   = $leaves->whereRaw($data[$column]." like '%".$string."%'")->whereBetween('date_from',
                        [$dateFrom, $dateTo])->paginate(20);
                } else {
                 ...
                 * First we build a query string which is common in both cases whether we have a condition set or not
                 */
                $leaves = DB::table('users')->select('users.id', 'users.name', 'employees.code', 'employee_leaves.days',
                    'employee_leaves.date_from', 'employee_leaves.date_to', 'employee_leaves.status', 'leave_types.leave_type',
                    'employee_leaves.remarks')->join('employees', 'employees.user_id', '=', 'users.id')->join('employee_leaves',
                    'employee_leaves.user_id', '=', 'users.id')->join('leave_types', 'leave_types.id', '=',
    ...
    public function approveLeave(Request $request)
    {
        ...
        $this->mailer->send('emails.leave_status',
            ['user' => $user, 'status' => 'approved', 'remarks' => $remarks, 'leave' => $employeeLeave], function ($message) use ($user) {
    ...
    public function disapproveLeave(Request $request)
    {
        ...
        $user          = User::where('id', $employeeLeave->user_id)->first();
        $this->mailer->send('emails.leave_status',
            ['user' => $user, 'status' => 'disapproved', 'remarks' => $remarks, 'leave' => $employeeLeave],
