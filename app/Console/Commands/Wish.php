...
class Wish extends Command
{
    ...
    public function handle()
    {
        ...
        $users     = Employee::whereRaw("DATE_FORMAT(`date_of_birth`, '%m-%d') = DATE_FORMAT('$dateToday', '%m-%d')")->with('user')->get();
        $emps      = Employee::whereRaw("DATE_FORMAT(`date_of_joining`, '%m-%d') = DATE_FORMAT('$dateToday', '%m-%d')")->with('user')->get();
